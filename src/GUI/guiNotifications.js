...
function sendNotification(type, text) {
  ...
  component.className = `relative flex items-center ${notificationType[type].color} text-titelColor text-sm font-bold px-4 py-3 rounded-md opacity-0 transform transition-all transition-[text] duration-500 mb-1`;
  component.innerHTML = `${notificationType[type].icon}<p>${text}</p>`;
  ...
  setTimeout(() => {
    ...
    //component.classList.add("-translate-y-80"); //it's a little bit buggy when send multiple notificationType
